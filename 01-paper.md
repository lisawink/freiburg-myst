...
| Dimension        | Volume | BuSA        | m²        | Building                | mean, std   || | 3d-building-metrics |
| Shape        | Hemisphericality | BuHem       | ratio        | Building                | mean, std     | $\frac{3 \sqrt{2 \pi} V}{A_{mesh}^{\frac{3}{2}}}$| Volume deviation between a polyhedron and itsequal-area hemisphere | 3d-building-metrics |
...
| Shape            | Fractality                       | BuFra_3D        | ratio     | Building                | mean, std               | $\mathrm{FR}_3=1-\frac{\log \left(V_{\mathrm{MS}}\right)}{\frac{3}{2} \times \log \left(A_{\mathrm{MS}}\right)}$ | Measures the edge roughness orsmoothness | 3d-building-metrics |
...
| Shape            | Equivalent Cuboid Index                          | BuECI_3D       | ratio     | Building                | mean, std               | $\left(\sqrt[3]{\frac{V_{\text{MS}}}{V_{\text{OOBB}}}}\right)^2 \times \frac{A_{\text{OOBB}}}{A_{\text{MS}}}$ | deviation of a polyhedron from an equivalent cuboid by scaling its minimum volume bounding box to match its volume | 3d-building-metrics |
...
| Shape            | Elongation            | BuElo_3D       | ratio     | Building                | mean, std               | | elongation of a polyhedron using its minimum bounding box along two axes: the shortest side against height and the longest side against height | 3d-building-metrics |
...
| **Metric**                  | **2D Definition**                                                                                       | **3D Definition**                                                                                       |
|-----------------------------|--------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|
| **Circularity / Hemisphericality** | Measures the area deviation between a polygon and its equal-perimeter circle.                        | Measures the volume deviation between a polyhedron and its equal-area hemisphere.                     |
| **Convexity**               | Measures the area deviation between a polygon and its convex hull, revealing curvature.                | Measures the volume deviation between a polyhedron and its convex hull, revealing curvature.           |
| **Fractality**              | Measures edge roughness or smoothness (Wentz, 2010).                                                  | Measures surface roughness or smoothness.                                                             |
| **Rectangularity / Cuboidness** | Measures the area deviation between a polygon and its minimum-area bounding rectangle.                | Measures the volume deviation between a polyhedron and its minimum-volume bounding box.               |
| **Squareness / Cubeness**   | Measures the perimeter deviation between a polygon and its equal-area square.                         | Measures the surface area deviation between a polyhedron and its equal-volume cube.                   |
| **Cohesion**                | Measures overall accessibility from all points to others within a polygon.                            | Measures overall accessibility from all points to others within a polyhedron.                         |
| **Proximity**               | Measures overall accessibility from all inner points to the center of a polygon.                      | Measures overall accessibility from all inner points to the center of a polyhedron.                   |
| **Exchange**                | Measures how much area inside a circle is exchanged with the area outside to create the polygon.      | Measures how much volume inside a sphere is exchanged with the volume outside to create the polyhedron.|
| **Spin**                    | Focuses on compactness of shape extremities.                                                          | Focuses on compactness of polyhedron extremities.                                                     |
| **Perimeter / Circumference** | Focuses on the compactness of a polygon’s boundary.                                                  | Focuses on the compactness of a polyhedron’s boundary.                                                |
| **Depth**                   | Highlights irregular changes along a polygon’s boundary.                                              | Highlights irregular changes along a polyhedron’s boundary.                                           |
| **Girth**                   | Measures the thickness of the layer insulating the core from the periphery.                          | Same, but applied to polyhedra.                                                                       |
| **Dispersion**              | Indicates whether a phenomenon propagates equally in all directions (Boyce and Clark).                | Adapts the 2D version to 3D propagation patterns.                                                     |
| **Range**                   | Measures the distance between the furthest edges of a polygon, sensitive to small, remote patches.    | Measures the distance between the furthest faces of a polyhedron, sensitive to remote sections.       |
| **Equivalent Rectangular / Cuboid Index** | Measures deviation of a polygon from an equivalent rectangle, addressing protrusion effects.     | Measures deviation of a polyhedron from an equivalent cuboid, addressing bounding box bias.           |
| **Equivalent Rectangular / Cuboid Index** | Measures deviation of a polygon from an equivalent rectangle by scaling its minimum area bounding rectangle to match its area. | Measures deviation of a polyhedron from an equivalent cuboid by scaling its minimum volume bounding box to match its volume. |
| **Roughness Index**         | Created as a measure of compactness. Advantages: (1) Less sensitive to elongation (aspect ratio of a polygon), and (2) more responsive to boundary roughness (intrusions and protrusions). | Created as a measure of compactness. Advantages: (1) Less sensitive to elongation (aspect ratio of a polyhedron), and (2) more responsive to boundary roughness (intrusions and protrusions). |
| **Elongation**              | Calculates the elongation of a shape using its minimum bounding rectangle. Based on Gil et al. (2009).      | Calculates the elongation of a polyhedron using its minimum bounding box along two axes: the shortest side against height and the longest side against height. |
| **Form Factor**             | –        
